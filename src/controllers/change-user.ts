...
class ChangeUserController {
  public async changeProfile(data: ChangeProfileFormModel) {
    ...
    try {
      const response = await changeUserProfileAPI.update() as XMLHttpRequest;
  ...
  public async changeAvatar(data: FormData) {
    ...
    try {
      const response = await changeUserAvatarAPI.update() as XMLHttpRequest;
      if (response.status === 200) {
        const data = JSON.parse(response.response);
        store.set('user', data);
      } else {
  ...
  public async changePassword(data: ChangePasswordFormModel) {
    ...
    try {
      const response = await changeUserPasswordAPI.update() as XMLHttpRequest;
      if (response.status === 200) {
        console.log('Ok');
      }
    } catch (error) {
